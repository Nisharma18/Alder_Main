  ...
  const fetchData = async () => {
    try {
      const [Stockdata] = await Promise.all([
        fetch("http://192.168.1.3:8085/Stocks", {
  ...
  const masterSearch = (e) => {
    ...
    if (1 == 1) {
      const filteredData = temptabledata.filter((item) => {
        const itemDate = new Date(item.inoutDate); // Use 'inoutDate' instead of 'date'
      ...
      for (const item of OUTData) {
        ...
        if (!OutAllQty[key]) {
          OutAllQty[key] = { ...item, qty: item.qty };
        } else {
          OutAllQty[key].qty += item.qty;
        }
      }
      const finalOutReceiveQty = Object.values(OutAllQty);
      settabledata(finalOutReceiveQty);
      ...
      for (const item of OUTData) {
        ...
        if (!Datewisepart[key]) {
          Datewisepart[key] = { ...item, qty: item.qty };
        } else {
  ...
  return (
    <React.Fragment>
      <div className="page-content" style={{overflow:"hidden"}}>
        ...
        />
        <Container fluid>
          <BreadCrumb title="Inventory Issue Register" pageTitle="MIS" buttons={
...
export default InventoryIssueRegister;
